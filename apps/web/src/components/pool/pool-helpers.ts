...
export function getPoolMetricChips(coin: CoinSummary, pool: PoolKey): Array<{ label: string; value: string }> {
  if (pool === "momentum") {
    return [
      ...
      { label: "评分", value: `${(coin.poolScores.momentum ?? 0).toFixed(1)}` },
    ];
  }
  if (pool === "trend") {
    return [
      ...
      { label: "评分", value: `${(coin.poolScores.trend ?? 0).toFixed(1)}` },
    ];
  }
  if (pool === "meanReversion") {
    return [
      ...
      { label: "评分", value: `${(coin.poolScores.meanReversion ?? 0).toFixed(1)}` },
    ];
  }
  return [
    { label: "方向", value: getPoolDirectionLabel(coin, pool) },
    { label: "OI", value: coin.openInterest ? formatCompactCurrency(coin.openInterest) : "—" },
    { label: "评分", value: `${(coin.poolScores.lsGame ?? 0).toFixed(1)}` },
  ];
}
